...
//const globby = async () => await import("globby");
...
const getPaths = async ({ distPath, exclude = [], cwd = "." }) => {
  ...
  const excludeFiles = exclude.map(
    (excludedPath) => `!${getRelPath(excludedPath, cwd).replace(/^!/, "")}`
  );
...
module.exports = async function makeSitemap(opts = {}) {
  ...
  const { sitemap, xml } = await createSitemapInfo({
    homepage,
    urls,
    failBuild,
  });
